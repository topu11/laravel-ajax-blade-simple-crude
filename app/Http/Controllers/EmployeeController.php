...
class EmployeeController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        else if(!in_array($request->file('avatar')->getClientOriginalExtension(),$accepted_extensions))
        {
            return response()->json([
                'message'=>'The uploaded File is not a image',
                'status' => 400
            ]);
        }
        else if($request->file('avatar')->getSize()>$accptedsize)
        {
            return response()->json([
                'message'=>'The uploaded File too large',
                'status' => 400
            ]);
        }
        else
